...
def get_similar_ids():
    inputed_text = 'company that is based in aarhus that works with bioengineering, make the research on biochemistry and creates the trials and drugs based on biomolecules'
...
#         top_ids = [ids[index] for index in top_indices]
#         companies = supabase.table('raw_companies_database').select("name", "size", "industry", "website").in_("id", top_ids).execute().data
